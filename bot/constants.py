BOT_START = "Starting bot...."
BOT_LOADING = "Loading: {}"
BOT_LOADING_EXTENTION = "Loading extention: {}"
BOT_FAILED_EXTENTION = "Failed to load extension {}\n{}"
LOADING_MESSAGE = "Loading..."
LOADING_DONE = ":white_check_mark:"
LOADING_DEFAULT_WHITELIST = "Default whitelist:\n{}"
LOGIN_MESSAGE = "Logged in as {0}\nID: {1}\nPrefix: {2}\nSkip Extentions: {3}"
LOADING_WHITELIST = "Whitelist:\n{}"
LOADING_ONLINELIST = "Onlinelist:\n{}"
LOADING_TIBIA_ONLINELIST = "Tibia.com Onlinelist:\n{}"
BOT_READY = "---------------\n{} is now ready to use!"
EMBED_BLANK = "\u200b"
LOADING_WHITELIST_MESSAGE = "Loading whitelist data...."
LOADING_ONLINELIST_MESSAGE = "Loading onlinelist data...."
TITLE_WHITELIST = "Whitelist"
TITLE_WHITELIST = "Onlinelist"
LODING_CHARACTER_DATA = "Loading player data...."
ERROR_LOADING_DATA = ":warning: Something went wrong :worried:"
CHARACTER_DATA_EMPTY = "You need to enter a player name"
TITLE_ONLINE_TIME = "Player Online Time"
DESCRIPTION_ONLINE_TIME = "[{name}]({url}) level {level} {voc} on {world}."
ERROR_MISSING_ARGUMENT = "You need to enter a player name."
EXP_MESSAGE = "Exp {}"
TITLE_EXPERIENCE_CHANGE = "Player Experience Change."
REMOVE_WHITELIST_MESSAGE = "Removed {name} {level} {voc} on {world} from whitelist."
ADD_WHITELIST_MESSAGE = "Added {name} {level} {voc} on {world} to whitelist."
HOUSE = "{name} in {town}"
GUILD = "[{guild}]({url})"
PLAYER_TITLE = "{name}"
LASTLOGIN_NONE = "never logged in"
EMBED_NONE = "None"
KILL_MESSAGE = "*{date}* Killed at level *{level}* by *{killers}* *{assists}*."
ONLINE_MESSAGE = "Level {level} {voc} on {world} went online."
OFFLINE_MESSAGE = "Level {level} {voc} on {world} went offline."
ONLINE_NONE = "No one online ::sob::"
LEVEL_ADVANCE_MESSAGE = "Advanced to level {level}."
HIGHSCORE_EXP_MESSAGE = "Experience\n***{0}***"
HIGHSCORE_MAGIC_MESSAGE = "Magic level\n***{0}***"
HIGHSCORE_DISTANCE_MESSAGE = "Distance\n***{0}***"
HIGHSCORE_SWORD_MESSAGE = "Sword Fighting\n***{0}***"
HIGHSCORE_AXE_MESSAGE = "Axe Fighting\n***{0}***"
HIGHSCORE_CLUB_MESSAGE = "Club Fighting\n***{0}***"
HIGHSCORE_SHIELDING_MESSAGE = "Shielding\n***{0}***"
HIGHSCORE_FIST_MESSAGE = "Fist Fighting\n***{0}***"
HIGHSCORE_FISHING_MESSAGE = "Fishing\n***{0}***"
HIGHSCORE_ACHIEVEMENTS_MESSAGE = "Achievement points\n***{0}***"
HIGHSCORE_LOYALTY_POINTS_MESSAGE = "Loyalty points\n***{0}***"
HIGHSCORE_RANK = "Rank: ***{0}***"
SHARE_MESSAGE = "A level {level} can share experience with level {min} to {max}."
SHARE_PLAYER_MESSAGE = "{name} level {level} can share experience with level {min} to {max}."
ACTIVITY_TITLE = "{online} | #tibia -help"
